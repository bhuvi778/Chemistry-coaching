...
const AIComparison = () => {
  const comparisonData = [
    {
      feature: "Chemistry Syllabus coverage",
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-20">
      ...
      <div className="relative">
        {/* Mobile Swipe Hint */}
        <div className="lg:hidden mb-3 flex items-center justify-center gap-2 text-cyan-400 text-sm font-medium animate-pulse">
          <i className="fas fa-hand-pointer rotate-90 transform"></i>
          <span>Swipe left to view full comparison</span>
          <i className="fas fa-chevron-right"></i>
        </div>

        <div className="glass-panel rounded-2xl overflow-hidden border border-gray-700 relative">
          <div className="overflow-x-auto pb-2"> {/* Added padding bottom for scrollbar */}
            <div className="min-w-[900px]">
              {/* Table Header */}
              <div className="grid grid-cols-4 gap-4 p-4 sm:p-6 bg-gray-900/50 border-b border-gray-700">
              ...
              <div className="divide-y divide-gray-700">
                {comparisonData.map((item, index) => (
                  <div key={index} className="grid grid-cols-4 gap-4 p-4 sm:p-6 hover:bg-gray-800/30 transition">
                    {/* Feature Name */}
                    <div className="flex items-center gap-3 text-white">
                      <i className={`${item.icon} text-cyan-400 text-lg`}></i>
                      <span className="font-medium text-sm sm:text-base">{item.feature}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-center">
                      {typeof item.coachingInstitutes === 'boolean' ? (
                        ...
                        <span className="text-gray-300 text-sm text-center font-medium">{item.coachingInstitutes}</span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-center">
                      {typeof item.chatGPT === 'boolean' ? (
                        ...
                        <span className="text-gray-300 text-sm text-center font-medium">{item.chatGPT}</span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-center">
                      {typeof item.prepiify === 'boolean' ? (
                        ...
                        <span className="text-cyan-400 font-bold text-sm text-center">{item.prepiify}</span>
                      )}
                    </div>
                  </div>
                ))}
          ...
          <div className="absolute top-0 right-0 h-full w-12 bg-gradient-to-l from-gray-900 via-gray-900/50 to-transparent pointer-events-none lg:hidden rounded-r-2xl"></div>
        </div>
      </div>
