...
const NCERTQuestions = () => {
  ...
  const handleBadgeClick = (badgeType) => {
    navigate(`/ncert-toolbox/questions/${badgeType}/questions`);
  };
    ...
    <div className="min-h-screen pt-32 pb-20">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-400 mb-8">
          ...
          <span className="text-white">NCERT Questions & Solutions</span>
        </div>

        {/* Header */}
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-pink-400 to-red-500 bg-clip-text text-transparent">
            NCERT Toolbox
          </h1>
          <p className="text-gray-400 text-lg max-w-2xl mx-auto mb-8">
            Practice NCERT questions organized by question types - MCQ, Subjective, and more
          </p>
        ...
        <div className="max-w-xl mx-auto mb-8">
          <div className="relative">
            <input
              ...
              className="w-full px-6 py-4 bg-gray-800/50 border border-gray-700 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:border-pink-500 transition"
            />
        ...
        {loading && (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 mt-4">Loading question types...</p>
          </div>
        )}
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
              {paginatedBadges.map((badge) => (
                <div
                  key={badge._id}
                  className="group glass-panel rounded-xl p-6 border border-gray-700 hover:border-pink-500/50 transition-all duration-300 hover:scale-105 cursor-pointer relative overflow-hidden"
                ...
                >
                  ...
                  <div className="relative z-10">
                    {/* Icon */}
                    <div className={`w-16 h-16 rounded-xl bg-${badge.color || 'pink'}-500/20 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}>
                      <i className={`fas ${badge.icon || 'fa-question-circle'} text-${badge.color || 'pink'}-400 text-2xl`}></i>
                    </div>

                    <div className="mb-3">
                      <span className="inline-block px-2.5 py-1 rounded-md text-xs font-semibold bg-pink-500/20 text-pink-400 border border-pink-500/30">
                        {badge.badgeType || 'Question Type'}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-gray-700">
                      ...
                      <i className="fas fa-arrow-right text-pink-400 group-hover:translate-x-1 transition-transform"></i>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {!loading && filteredBadges.length === 0 && (
          <div className="text-center py-20 glass-panel rounded-xl border border-gray-700">
            ...
            <p className="text-gray-400">
              {searchQuery ? `No question types found matching "${searchQuery}"` : 'Admin has not added any question types yet.'}
            </p>
          </div>
        )}
      </div>
