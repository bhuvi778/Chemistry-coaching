...
const ManageCrosswords = () => {
    ...
    const handleEdit = (crossword) => {
        ...
        setFileNames({
            thumbnail: crossword.thumbnailUrl ? 'Current thumbnail' : '',
            setPdf: crossword.setPdfUrl ? 'Current PDF' : '',
            answerPdf: crossword.answerPdfUrl ? 'Current PDF' : ''
        });
    };

    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this crossword set?')) {
            try {
                const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000';
                await fetch(`${API_URL}/api/crosswords/${id}`, { method: 'DELETE' });
                fetchCrosswords();
        ...
        <div className="space-y-8">
            <div className="glass-panel p-6 rounded-xl">
                <h2 className="text-xl font-bold text-white mb-4">
                    {isEditing ? 'Edit Crossword Set' : 'Add New Crossword Set'}
                </h2>
            ...
            <div className="grid grid-cols-1 gap-4">
                <h3 className="text-xl font-bold text-white">All Crossword Sets ({crosswords.length})</h3>
                {loading ? (
                    ...
                    <div className="text-center py-10 glass-panel rounded-xl">
                        ...
                        <p className="text-gray-400">No crossword sets added yet</p>
                    </div>
                ) : (
                    crosswords.map(crossword => (
                        <div key={crossword._id} className="glass-panel p-4 rounded-xl flex justify-between items-center">
                            <div className="flex-1">
                                <h3 className="text-lg font-bold text-white">{crossword.setNumber} - {crossword.title}</h3>
